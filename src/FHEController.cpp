...
vector<Ptxt> FHEController::read_plain_256_input(const string& filename, int level, double scale) {
    vector<double> input = read_values_from_file(filename);

    vector<double> input_0;
    ...
    for (int i = 0; i < 128; i++) {
        input_0.push_back(input[i]);
    }

    for (int i = 128; i < 256; i++) {
        input_1.push_back(input[i]);
    }

    res.push_back(context->MakeCKKSPackedPlaintext(input_0, 1, level, nullptr, num_slots));
    res.push_back(context->MakeCKKSPackedPlaintext(input_1, 1, level, nullptr, num_slots));
