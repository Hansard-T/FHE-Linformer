...
class BBC_News(Dataset):
    ...
    def __add__(self, other):
        ...
        return self
    def get_sub_set(self, indexs):
        # Avoid creating a ragged numpy array from sequences; use list comprehension
        idx_list = indexs.tolist() if hasattr(indexs, 'tolist') else list(indexs)
        sequences = [self.sequences[i] for i in idx_list]
        sLens = [self.sLens[i] for i in idx_list]
        labels = [self.labels[i] for i in idx_list]
        obj = BBC_News_(self.filePath, sequences, sLens, labels)
        return obj
