    ...
    def vectorize(self, sequences, method=["skipgram"], embSize=64, window=7, iters=10, batchWords=10000,
        ...
        if os.path.exists(path) and loadCache:
            ...
            if 'cbow' in method:
                ...
                embeddings.append(word2vec)
            if 'glove' in method:
                print("Building GloVe co-occurrence matrix...")
                
                def build_cooccurrence(sentences, token_to_id, vocab_size, win):
                    counts = defaultdict(float)
                    for seq in sentences:
                        n = len(seq)
                        for i, wi in enumerate(seq):
                            if wi not in token_to_id:
                                continue
                            id_i = token_to_id[wi]
                            left = max(0, i - win)
                            right = min(n, i + win + 1)
                            for j in range(left, right):
                                if j == i:
                                    continue
                                wj = seq[j]
                                if wj not in token_to_id:
                                    continue
                                id_j = token_to_id[wj]

                                counts[(id_i, id_j)] += 1.0 / abs(j - i)

                    rows, cols, data = zip(*[(i, j, v) for (i, j), v in counts.items()])
                    cooc = sp.coo_matrix((data, (rows, cols)), shape=(vocab_size, vocab_size), dtype=np.float64)
                    cooc = cooc + cooc.T
                    return cooc

                cooc = build_cooccurrence(corpus, self.tkn2id, self.tknNum, window)
                cooc = cooc.toarray()

                glove_model = GloVe(n=embSize, max_iter=iters)
                word_vectors = glove_model.fit(cooc)

                word2vec = np.zeros((self.tknNum, embSize), dtype=np.float32)
                for i in range(self.tknNum):
                    word2vec[i] = word_vectors[i]

                embeddings.append(word2vec)
                print("✅ Finished training GloVe embeddings.")
